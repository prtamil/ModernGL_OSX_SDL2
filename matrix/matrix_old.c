...
int main()
{
        ...
        glVertexAttribPointer(1, 4, GL_FLOAT, GL_FALSE, 16, (void*)48);
        ...
        while(gameloop_running){
                ...
                mat4x4_translate_in_place(view, 0.1f, -0.2f, -2.0f);
                mat4x4_perspective(proj, 60.0f, 480.0/640.0, 0.01f, 100.0f);

                GLuint mloc = glGetUniformLocation(shader_program, "modelMatrix");
                GLuint vloc = glGetUniformLocation(shader_program, "viewMatrix");
                GLuint ploc = glGetUniformLocation(shader_program, "projMatrix");



                glUseProgram(shader_program);
                glProgramUniformMatrix4fv(shader_program, mloc, 1, GL_FALSE, &model[0][0]);
                glProgramUniformMatrix4fv(shader_program, vloc, 1, GL_FALSE, &view[0][0]);
                glProgramUniformMatrix4fv(shader_program, ploc, 1, GL_FALSE, &proj[0][0]);
                //drawing code 
                ...
                mat4x4_identity(view);
                mat4x4_translate_in_place(view, 0.1f, -10.0f, -2.0f);
                glProgramUniformMatrix4fv(shader_program, vloc, 1, GL_FALSE, &view[0][0]);
